...
def test_fields(fields):
    ...
    concatenated = ''.join(_to_str(_resolve(txn, f)) for f in sorted_fields)
    computed = hmac.new(
    ...
    if computed == expected:
        print("FOUND MATCH!")
        print("Fields:", sorted_fields)
        return True
    return False

# Paymob often silently adds `is_refund` or `is_settled` to the HMAC without updating docs
candidate_extras = [
    'is_refund', 'is_settled', 'merchant_order_id', 'amount_cents_int', 'captured_amount',
    'refunded_amount_cents', 'is_live'
]

# Try all combinations of up to 3 extra fields
for i in range(4):
    for extras in itertools.combinations(candidate_extras, i):
        if test_fields(base_fields + list(extras)):
            import sys
            sys.exit(0)
