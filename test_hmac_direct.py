...
# Typical fields + new fields
possible_fields = {
    'amount_cents': txn.get('amount_cents'),
    'created_at': txn.get('created_at'),
    'currency': txn.get('currency'),
    'error_occured': txn.get('error_occured'),
    'has_parent_transaction': txn.get('has_parent_transaction'),
    'id': txn.get('id'),
    'integration_id': txn.get('integration_id'),
    'is_3d_secure': txn.get('is_3d_secure'),
    'is_auth': txn.get('is_auth'),
    'is_capture': txn.get('is_capture'),
    'is_refunded': txn.get('is_refunded'),
    'is_standalone_payment': txn.get('is_standalone_payment'),
    'is_void': txn.get('is_void'),
    'is_voided': txn.get('is_voided'),
    'order': txn.get('order', {}).get('id'),
    'owner': txn.get('owner'),
    'pending': txn.get('pending'),
    'source_data_pan': txn.get('source_data', {}).get('pan'),
    'source_data_sub_type': txn.get('source_data', {}).get('sub_type'),
    'source_data_type': txn.get('source_data', {}).get('type'),
    'success': txn.get('success'),
}

# Paymob HMAC is simply these keys, lexically sorted, concatenated
sorted_keys = sorted(list(possible_fields.keys()))
concatenated = ''.join(_to_str(possible_fields[k]) for k in sorted_keys if possible_fields[k] is not None)

secret = "D0AFA2B8C5CD1CAE78A9B8F0065BFDD8"
expected = "2c2a752cd1832a0f4ce85a0995b461d174bb9dcfcafcaa9d2a293b136d81f62a79a1a287b998fa3d0cdde19a23d83bd49ac4c1d07163d9ea2624a56d3ebae9bf"
